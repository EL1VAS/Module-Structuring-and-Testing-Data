...
function isSixteenDigitNumber(cardNumber) {
    ...
    if (cardNumberLength === 16) { // Check if the length is exactly 16
        for (let i = 0; i < cardNumberLength; i++) { //Check each digit till the length of cardNumber
...
function card-validator(cardNumber) {
    if (isSixteenDigitNumber(cardNumber) === true) {    
        if hasTwoDifferentDigits(cardNumber) && lastDigitIsEven(cardNumber) && sumOfDigitsGreaterThan16(cardNumber) {
